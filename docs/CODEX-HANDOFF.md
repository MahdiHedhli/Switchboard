...
- local-only shells with no exported `SWITCHBOARD_OPERATOR_TOKEN` or `SWITCHBOARD_OPERATOR_TOKEN_FILE` now also surface as an explicit fail-closed operator state: the direct and preflight operator `message` becomes `Local-only mode should set SWITCHBOARD_OPERATOR_TOKEN.`, `operatorTokenSource` reports `unset`, and mutation scopes remain `open` until shell-side token wiring is added
